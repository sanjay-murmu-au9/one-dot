...
export function drawTimerOverlay(ctx, w, h, color) {
  const now = new Date()
  ...
  // 1. Draw Date (Premium small text)
  setFont(ctx, '600', w * 0.045)
  ctx.fillStyle = color
  ctx.globalAlpha = 0.85
  ctx.fillText(date.toUpperCase(), w / 2, h * 0.16)

  // 2. Draw Time (Bold large lock-screen clock)
  setFont(ctx, '700', w * 0.22, 'system-ui, -apple-system, sans-serif')
  ctx.fillStyle = color
  ctx.globalAlpha = 1
  ctx.shadowBlur = 10
  ctx.shadowColor = 'rgba(0,0,0,0.1)'
  ctx.fillText(time, w / 2, h * 0.3)
