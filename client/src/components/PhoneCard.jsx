...
import { DRAW_FUNCTIONS, STYLE_ACCENTS, drawTimerOverlay, loadHourglassImage } from './WallpaperCanvas'
  ...
  useEffect(() => {
    ...
    loadHourglassImage().then(hourglassImg => {
      const drawWithOverlay = (bgImg = null) => {
        drawFn(ctx, width, height, 128, accent, bgImg)
        // Draw timer overlay with hourglass and progress bar
        const overlayColor = (styleId === 'memento-mori' || styleId === 'dot-grid') ? accent : '#ffffff'
        // For thumbnails, don't pass density (hourglass won't show in gallery)
        drawTimerOverlay(ctx, width, height, overlayColor, hourglassImg, styleId, '')
      }
